...
Logic Changes (v15):
- Push Days: Correctly defined as "Last occurence of Mon-Sat".
- BU Compaction: Post-processing step ensures BU roles always fill BU1->BU4 sequentially.
- Strict Constraints: Separation of Holiday vs Push week logic.
- Input Hardening: Stricter regex, date parsing, and validation.
"""
...
def load_sales_ranking_from_timeoff_xlsx(xlsx_path, sheet_name, year, month, roster_names):
    ...
    # Improved Regex for flexible quarter format
    def key(x):
...
def push_days_for_month(year: int, month: int) -> Set[date]:
    ...
    # Find last occurrence of each weekday 0-5 (Mon-Sat)
    for wd in range(6): 
        cur = last
...
def build_schedule(config: BuildConfig,
    ...
    def week_contains_push(ws):
        for i in range(7):
            ...
            if dd in push_days: return True
        return False

    def week_contains_holiday(ws):
        for i in range(7):
            dd = ws + timedelta(days=i)
            if dd in hols: return True
        return False
    ...
    def can_assign_primary(d, role, name, strict_mode):
        ...
        # Weekly Hours (Relax on Holiday OR Push)
        ws = week_start_sun(d)
        is_relaxed_week = week_contains_holiday(ws) or week_contains_push(ws)
        
        if not is_relaxed_week:
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        for i, sd in enumerate(sundays):
            ...
            if can_assign_primary(sd, "PN", who, strict_mode=False):
