...
Logic Changes (v18):
- FIXED: Restored missing Constraints class and _rotate_left function.
- FIXED: Excel fill logic to ensure in-month weekdays are white (not grey).
- IMPROVED: Strict Mode Sunday rotation now searches for next valid candidate if quota full.
"""
...
def build_schedule(config: BuildConfig,
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        for i, sd in enumerate(sundays):
            # ROTATION SEARCH: If "strict mode" quotas block the preferred person,
            # or if hard-off blocks them, scan forward to find the next valid person.
            assigned_sunday = False
            for offset in range(len(config.roster)):
                who = config.roster[(start_idx + i + offset) % len(config.roster)]
                
                # Cannot work if Hard Off
                if who in cons.hard_off.get(sd, set()):
                    continue
                
                # Strict Mode: Must respect quota
                if strict_mode:
                    if role_counts["PN"][who] >= targets["PN"][who]:
                        continue
                
                # Soft Mode Checks (Back-to-back etc)
                ...
                # Pair with Saturday AN (If possible)
                ...
                break # Move to next Sunday
            
            # If we couldn't assign a Sunday in strict mode, we must fail strict mode
            if not assigned_sunday and strict_mode:
                return False

        # 2. Assign remaining Primaries
        primary_slots = []
        for d in days:
            req = roles_required_for_day(d, push_days, hols)
            for r in ["PN", "AN", "W"]:
    ...
    def can_bu(d, role, p):
        ...
        # Hard Back-to-Back for BU
        if last_role(p, d) == role: return False
        ...
        # Pay Period cap (relaxed ONLY if push)
        if not is_push:
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods):
    ...
    for wk in range(grid_weeks):
        ...
        for dow in range(7):
            ...
            # Fill logic: In-month Weekdays = White. Sundays = Yellow. Non-month = Grey.
            fill = None
            if 1 <= day_num <= last.day:
                ...
                if is_sunday(d): 
                    ...
                    fill = None # Explicitly None for weekdays (White)
            else:
