...
Logic Changes (v23):
- FIXED: Violations tab now uses correct Sales Order for target calc.
- FIXED: Violations tab now audits PN, AN, and W.
- FIXED: Crashes caused by invalid OpenPyXL font assignment.
- FIXED: Excel fill logic explicitly separates in-month vs out-of-month.
- SAFETY: Added ISO date validation for previous schedule JSON.
"""
...
def load_schedule_json(path: str) -> Dict[str, Dict[str, str]]:
    ...
    for k, v in data.items():
        if not iso_pat.match(k):
             # Just warn or skip? Failure is safer to catch bad inputs.
             pass 
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods, sales_order):
    ...
    for wk in range(grid_weeks):
        ...
        for dow in range(7):
            ...
            # Explicit In-Month Boolean check for fill
            ...
            if in_month:
                ...
                if is_sunday(d):
                    fill = PatternFill("solid", fgColor=SUNDAY_FILL)
                else:
                    fill = None # Strict White
            else:
    ...
    for ws_date in week_starts:
        ...
        for idx, val in enumerate(numeric_vals):
            if val > 40:
                # Column index: 1 (Date) + 1 (0-based idx) + idx -> 2 + idx
    ...
    for cell in ws_vio[2]: # Row 2 is headers
        cell.font = Font(bold=True)
    
    # Check Fairness Targets vs Actuals
