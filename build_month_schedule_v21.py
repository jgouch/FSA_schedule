...
- Exports:
    ...
    - Excel calendar workbook (Calendar, Summary, Weekly Hours, Pay Period Hours)
...
Logic Changes (v21):
- FIXED: Added missing "Weekly Hours" tab to Excel export.
- OPTIMIZED: Implemented true caching for MRV solver candidate counts.
- ROBUST: Normalized Sales Ranking keys to handle spaces ("2026 Q1" -> "2026Q1").
- CONSISTENT: Weekly caps relax on Holiday OR Push; PP caps relax on Push only.
"""
...
def load_sales_ranking_from_timeoff_xlsx(xlsx_path, sheet_name, year, month, roster_names):
    ...
    for r in range(2, ws.max_row + 1):
        p = ws.cell(r, 1).value
        if not p: continue
        p = normalize_quarter_key(str(p)) # Handle "2026 Q1"
        ranks = []
...
def build_schedule(config: BuildConfig,
    ...
    # Move can_bu here so it can be used in queue gen
    def can_bu(d, role, p):
        if d in hols: return False
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        def count_candidates(slot):
            if slot in cand_cache: return cand_cache[slot]
            d, r = slot
            v = sum(1 for n in config.roster if can_assign_primary(d, r, n, strict_mode))
            cand_cache[slot] = v
            return v
        ...
        def backtrack(slots_left):
            ...
            # Use cached counts to speed up MRV
            ...
            for p in cands:
                ...
                # Invalidate cache for slots that might be affected?
                # Actually, strictly, assignment changes constraints.
                # So we must clear cache or not use it for recursion.
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods):
    ...
    for ws_date in week_starts:
        ...
        for p in roster:
            ...
            c = ws_weekly.cell(row=ws_weekly.max_row+1, column=len(row)+1, value=tot)
            if tot > 40:
                c.fill = ot_fill
            row.append(tot) # Logic handled by cell assignment, just tracking index here
