...
Logic Changes (v17):
- Fairness Math: Fixed "Rolling Extras" to use remainders, not totals.
- Safety: Priority list strictly enforces full roster inclusion.
- Strict Mode: Sunday rotation now respects quotas in Pass 1.
- BU Rules: Hard enforcement of Back-to-Back and Pay-Period caps.
"""
...
def compute_primary_targets(days, push_days, roster, sales_order, hols):
    ...
    # Priority list must include EVERYONE
    ...
    def distribute(role, total):
        ...
        # Remainder-based rolling offset
        offset = 0
        rem_pn = opp["PN"] % n
        rem_an = opp["AN"] % n
...
def build_schedule(config: BuildConfig,
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        for i, sd in enumerate(sundays):
            ...
            # STRICT MODE: Enforce quotas on Sunday assignments too
            if strict_mode:
    ...
    def can_bu(d, role, p):
        ...
        # Weekly cap (relaxed if holiday or push)
        ...
        # FIX: Pay Period cap (relaxed ONLY if push)
        if not is_push:
