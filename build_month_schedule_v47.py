...
def weekday_staff_metrics(
    ...
    total_weekdays_checked = 0
    days_with_target_or_more = 0
    min_assigned_on_weekday: Optional[int] = None
    deficit_sum_to_target = 0

    for d in month_days(year, month):
        if d in hols or not is_weekday(d):
            continue
        total_weekdays_checked += 1
        assigned_count = len(set(schedule.get(d.isoformat(), {}).values()))
        if min_assigned_on_weekday is None or assigned_count < min_assigned_on_weekday:
            min_assigned_on_weekday = assigned_count
        if assigned_count >= target:
            days_with_target_or_more += 1
        deficit_sum_to_target += max(0, target - assigned_count)

    return {
        "total_weekdays_checked": total_weekdays_checked,
        "days_with_target_or_more": days_with_target_or_more,
        "days_with_5_or_more": days_with_target_or_more,
        "min_assigned_on_weekday": min_assigned_on_weekday if min_assigned_on_weekday is not None else 0,
        "deficit_sum_to_target": deficit_sum_to_target,
    }
...
def main():
    ...
    ap.add_argument("--seed-end", type=int, default=5000)
    ...
    if not args.auto_seed:
        ...
        for seed in range(args.seed_start, args.seed_end + 1):
            cfg = BuildConfig(year, month, roster, sales, rng_seed=seed)
            ...
            score = (
                metrics["days_with_target_or_more"],
                -metrics["deficit_sum_to_target"],
            )
        ...
        if sched is None:
            raise RuntimeError(
                f"Auto-seed failed for range [{args.seed_start}, {args.seed_end}]. Last failure: {last_failure_reason}"
            )

        print(f"✅ Auto-seed success: seed={winning_seed}")
        if args.optimize_weekday_staff and winning_metrics is not None:
            print(
                f"Weekday staffing: {winning_metrics['days_with_target_or_more']}/"
                f"{winning_metrics['total_weekdays_checked']} weekdays meet target={args.target_weekday_staff}, "
                f"min weekday assigned={winning_metrics['min_assigned_on_weekday']}"
            )
