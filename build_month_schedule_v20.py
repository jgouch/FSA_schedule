...
Logic Changes (v20):
- FIXED: Excel fill logic (Weekdays = White, Non-Month = Grey).
- FIXED: BU Feasibility now uses real can_bu() check before queueing.
- OPTIMIZED: MRV solver uses caching for candidate counts.
- SAFETY: Stronger validation for inputs.
"""
...
def load_schedule_json(path: str) -> Dict[str, Dict[str, str]]:
    ...
    for k, v in data.items():
        ...
        # Optional: check if K is ISO date
        try: datetime.strptime(k, "%Y-%m-%d")
        except: pass # Allow flexibility but warn if needed
    return data
...
def build_schedule(config: BuildConfig,
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        def backtrack(slots_left):
            ...
            # Use cached counts if you were caching, but re-computing for min is okay for small N
            # Optimization: pick best slot (min candidates)
            ...
            # Simple MRV cache context could be here, but direct approach is stable
            for p in cands:
    ...
    for d in days:
        ...
        for rname in ideal:
            ...
            # FEASIBILITY CHECK:
            # Does anyone in temp_pool actually qualify?
            # We must pass the pool candidates to can_bu
            # Note: temp_pool contains names. can_bu checks constraints.
            valid_cands = [p for p in temp_pool if can_bu(d, rname, p)]
            
            if not valid_cands:
                # No one can fill this slot properly.
                # Do we skip? Yes, to avoid empty slot or hang.
                # Do we consume a person? No, if we skip, we don't use a resource.
                continue

            bu_queue.append((d, rname))
            # Heuristic: Remove one person from pool. 
            # Ideally remove one of the VALID ones.
            # We pick first valid to be deterministic.
