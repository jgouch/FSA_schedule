...
def build_schedule(config: BuildConfig,
    ...
    if payperiods:
        ...
        # Note: We now allow payperiods to extend beyond month end for display logic,
        # so we don't strictly validate end > last_day here anymore.
    ...
    def candidates_for(slot: Tuple[date, str]) -> List[str]:
        ...
        rng.shuffle(cands) # FIX 1: Seeded shuffle
    ...
    for d in days:
        ...
        # 2. Strict Sequential Filling (FIX 2 & 3: Deterministic & Skip Impossible)
        # We try to fill BU roles in order. If BU1 is IMPOSSIBLE (no one valid), we SKIP IT.
        # This prevents the solver from dying if everyone available has "Avoid BU1".
        
        assigned_today = set(schedule[d.isoformat()].values())
        hard_off_today = cons.hard_off.get(d, set())
        
        # Initial pool of available people (FIX 2: SORTED for determinism)
        available_pool = sorted([p for p in roster if p not in assigned_today and p not in hard_off_today])
        
        slots_to_create = []
        temp_pool = available_pool[:]  # Copy list
        ...
        for role_name in ideal_bu:
            ...
            if not valid_cands:
                # FIX 3: If we can't fill BU1, don't break. SKIP IT and try BU2.
                # This matches "Maximize Headcount" preference.
                continue
            
            # We assume we can fill it. Add to list.
            slots_to_create.append(role_name)
            
            # "Reserve" one person (heuristically) to ensure we don't over-promise capacity.
            # FIX 2: Pick first deterministic candidate
    ...
    def can_assign_bu(d: date, role: str, name: str) -> bool:
        ...
        if name in cons.avoid_roles.get((d, role), set()): return False
        if name in schedule[d.isoformat()].values(): return False
        # REMOVED: prev_role check strictness. Back-to-back allowed for BU.
        if consecutive_streak(name, d) >= config.max_consecutive_days: return False
        if would_exceed_week_cap(d, role, name): return False
        if would_exceed_week_days_cap(d, name): return False
        if would_exceed_payperiod_cap(d, role, name): return False
    ...
    def score_bu(d: date, role: str, name: str) -> float:
        ...
        # Penalize back-to-back same role, but allow it if necessary
        if last_role(name, d) == role:
            s -= 50.0
