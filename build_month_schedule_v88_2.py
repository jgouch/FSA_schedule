...
Logic Changes (v88.2):
- FIX (v88.2): PN sales-rank preference pass clamps PN counters to prevent negative values during feasibility simulation.
- CLEANUP (v88.2): Remove redundant unconditional return in push-week BU Saturday rule.
            ...
            for hi in pn_priority:
                for lo in pn_priority:
                    ...
                    for d in days:
                        ...
                        hrs = hours_for_role(d, "PN", hols)
                        wd = weekday_sun0(d)
                        role_counts["PN"][lo] = max(0, role_counts["PN"][lo] - 1)
                        total_hours[lo] -= hrs
                        pn_weekday_counts[lo][wd] = max(0, pn_weekday_counts[lo][wd] - 1)
                        if wd == 1:
