#!/usr/bin/env python3
# schedule_logic_v3.py (dynamic staffing + backtracking)
...
def roles_for_day(d: date, inp: Inputs, push_days: Set[date]) -> List[str]:
    ...
    if d.weekday() == 0:
        ...
        desired_total = min(6 if (inp.push_week_enabled and d in push_days) else 5, cap)
...
class Solver:
    ...
    def can_assign(self, p: str, d: date, role: str) -> Tuple[bool, str]:
        if d in self.inp.time_off.get(p, set()):
        ...
        if role == "PN" and d.weekday() == 0 and self.mon_pn[p] >= self.inp.monday_pn_max_per_month:
    ...
    def enforce_sat_an_for_sun(self):
        for d in self.days:
            if is_sunday(d) and self.sched[d].get("PN"):
                p = self.sched[d]["PN"]
                sat = d - timedelta(days=1)
                if sat.month != self.inp.month or sat in self.inp.observed_holidays:
                    continue
                req_sat = roles_for_day(sat, self.inp, self.push_days)
                if "AN" not in req_sat:
                    continue
    ...
    def solve(self, max_nodes: int = 1_200_000) -> Dict[date, Dict[str, str]]:
...
if __name__ == "__main__":
    ...
    sched = solver.solve(max_nodes=1_200_000)
