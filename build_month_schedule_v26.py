...
Logic Changes (v26):
- FIXED: Removed extra argument in 'compute_primary_targets' call (Crash fix).
- AUDIT: Distinguishes "Hard Violation" (Primary Avoid) vs "Soft Warning" (BU Avoid).
- AUDIT: Added Weekly/PP Hours check to Violations tab (respecting relaxations).
- REFACTOR: Shared 'week_contains' logic to ensure Solver and Audit match.
"""
...
# Global logic checks (Shared between Solver and Audit)
...
def build_schedule(config: BuildConfig,
    ...
    # FIX: Corrected arguments (removed push_days)
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods, sales_order, cons):
    ...
    # 1. Target Audit
    # Fix: Removed extra push_days arg
    ...
    for d_iso, rmap in schedule.items():
        ...
        for r, p in rmap.items():
            ...
            if p in avoid_list:
                # Distinguish Soft (BU) vs Hard (Primary)
                if r in ROLE_PRIMARY:
                    ws_vio.append(["Avoid Violation (Hard)", p, f"Assigned {r} on {d}"])
                else:
                    ws_vio.append(["Avoid Warning (Soft)", p, f"Assigned {r} on {d}"])
        ...
        for r, p in rmap.items():
            if prev_roles.get(r) == p:
                ws_vio.append(["Back-to-Back", p, f"Worked {r} on {prev_d} and {d}"])

    # 3. Hours Audit (Weekly + PP)
    push_days = push_days_for_month(year, month)
    ...
    if payperiods:
        for s, e in payperiods:
            # Check if PP intersects a Push Week. 
            # Logic: if any day in PP is a Push Day, relax? Or if any week in PP is push?
            # Build rule: Relax PP cap if any week in it is a push week.
            # Simplified: Check if any day in PP is push.
            pp_is_push = False
            cur = s
            while cur <= e:
                if cur in push_days: 
            ...
            if pp_is_push: continue

            for p in roster:
                # Recalc PP hours using helper
                h = 0
                cur = s
                while cur <= e:
                    h += hours_for_any_role_on_day(cur, p)
                    cur += timedelta(days=1)
