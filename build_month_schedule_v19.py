...
Logic Changes (v19):
- CLEANUP: Removed all duplicate function/class definitions.
- OPTIMIZATION: Used min() instead of sort() in MRV solver.
- STABILITY: Added feasibility check before enqueuing BU slots.
"""
...
def build_schedule(config: BuildConfig,
    ...
    for d in days:
        ...
        for rname in ideal:
            ...
            # Feasibility Check: Is there ANYONE who can take this?
            # We relax back-to-back for this check to ensure we don't skip valid slots
            # that might just have a soft penalty.
            # But hard constraints (hard off, assigned) are already filtered from temp_pool.
            # So just check if temp_pool is not empty.
    ...
    def can_bu(d, role, p):
        ...
        if last_role(p, d) == role: return False # Hard B2B for BU (User request)
