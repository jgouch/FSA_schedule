...
def build_schedule(config: BuildConfig,
    ...
    def can_assign_primary(d, role, name, strict_mode):
        ...
        if role == 'AN' and is_saturday(d) and d.day == calendar.monthrange(year, month)[1] and consecutive_streak(name, d) >= (config.max_consecutive_days - 1): return False
    ...
    def can_bu(d, role, p):
        ...
        if prev_d.month != month:
            prev_roles = prev_sched.get(prev_d.isoformat(), {})
        # BU back-to-back is a SOFT preference only (see bu_score penalties).
