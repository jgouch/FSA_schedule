...
Logic Changes (v22):
- FIXED: Weekly Hours tab now aligns rows correctly.
- NEW: Added "Violations" tab for target deviations.
- ROBUST: Full Sales Ranking normalization.
- CLEAN: Removed redundant solver cache for stability.
"""
...
def load_sales_ranking_from_timeoff_xlsx(xlsx_path, sheet_name, year, month, roster_names):
    ...
    ws = wb[sheet_name]
    # FIXED: Normalize target key as well
...
def build_schedule(config: BuildConfig,
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        def backtrack(slots_left):
            ...
            # Simple MRV optimization (no cache needed for small N)
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods):
    ...
    ws_vio.font = Font(bold=True)
    ws_vio.append(["Type", "Person", "Details"])
    
    # Check Fairness Targets vs Actuals
    # Re-compute targets locally to compare
    targets = compute_primary_targets(month_days(year, month), push_days_for_month(year, month), roster, [], hols)
    # Re-count actuals
    real_counts = {p: 0 for p in roster}
    for d_iso, rmap in schedule.items():
        p = rmap.get("PN")
        if p: real_counts[p] += 1
        
    for p in roster:
        tgt = targets["PN"][p]
        act = real_counts[p]
        if act != tgt:
            ws_vio.append(["Target Mismatch", p, f"PN Count {act} != Target {tgt}"])
