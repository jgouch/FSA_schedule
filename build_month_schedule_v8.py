...
# STRICT 5-week geometry per user requirement (Rows 3..42)
MAX_WEEKS = 5  
...
def pick_week_index_for_day(year: int, month: int, day: int) -> Tuple[int, int, int, int]:
    ...
    Strictly enforcing MAX_WEEKS = 5.
    """
...
def build_schedule(config: BuildConfig,
    ...
    # Check for 6-week month violation early
    first_day = date(year, month, 1)
    last_day_num = calendar.monthrange(year, month)[1]
    offset = weekday_sun0(first_day)
    weeks_needed = (offset + last_day_num + 6) // 7
    if weeks_needed > MAX_WEEKS:
        raise ValueError(
            f"Month {year}-{month:02d} spans {weeks_needed} weeks, but the calendar grid "
            f"is strictly locked to {MAX_WEEKS} weeks. This month cannot be generated "
            "without altering the 'LOCKED CALENDAR GEOMETRY' or Excel template."
        )
    ...
    # This replaces the old "roles_required_for_day" loop for BU.
    # We now check how many people are actually available and only create that many slots.
    ...
    for d in days:
        ...
        # 2. Determine who is actually available (not already assigned, not hard-off)
        assigned_today = set(schedule[d.isoformat()].values())
        hard_off_today = cons.hard_off.get(d, set())
        
        available_count = 0
        for p in roster:
            if p in assigned_today: continue
            if p in hard_off_today: continue
            available_count += 1

        # 3. Generate only the slots we can physically fill
        # We take the first N roles from ideal_bu, where N is available_count
        count_to_fill = min(len(ideal_bu), available_count)
        
        for i in range(count_to_fill):
            role_name = ideal_bu[i]
            # Safety check (though primary phase handles it)
