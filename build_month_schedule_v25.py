...
Logic Changes (v25):
- FIXED: Violations tab now uses passed 'cons' object (no file reloading).
- FIXED: Implemented actual Hard Off and Avoid Role audits in output.
- CLEANUP: Removed unused 'push_days' from target computation.
- VISUAL: Retained v24's White/Grey fill logic for correct appearance.
"""
...
def build_schedule(config: BuildConfig,
    ...
    # CLEANUP: Removed unused push_days argument
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods, sales_order, cons):
    ...
    # 2. Hard Constraints Audit (Using passed 'cons' object)
    for d_iso, rmap in schedule.items():
        ...
        for r, p in rmap.items():
            ...
            if p in avoid_list:
                ws_vio.append(["Avoid Role Violation", p, f"Worked {r} on {d} (Asked to Avoid)"])

        # Back-to-Back Check (Strict Role Matching)
        prev_d = d - timedelta(days=1)
        prev_roles = schedule.get(prev_d.isoformat(), {})
        # Also check prev_sched if month boundary?
        if prev_d.month != month:
             # Just a simple heuristic if in prev_sched
             # Note: schedule keys are iso strings
