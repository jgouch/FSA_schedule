...
def build_schedule(config: BuildConfig,
    ...
    def can_assign_primary(d, role, name, strict_mode):
        ...
        if (not is_push_week) and consecutive_streak(name, d) >= config.max_consecutive_days: return False
    ...
    def can_bu(d, role, p):
        ...
        if (not is_push) and consecutive_streak(p, d) >= config.max_consecutive_days: return False
    ...
    # print("Warning: Could not fill all desired BU slots.")
    # --- Greedy fallback BU fill (best effort) ---
    # If the global BU backtracking can't satisfy every desired BU slot,
    # we still fill what we can day-by-day instead of leaving BU entirely blank.
    for d, role in bu_queue:
        d_iso = d.isoformat()
        if role in schedule[d_iso]:
            continue
        cands = [pp for pp in config.roster if can_bu(d, role, pp)]
        if not cands:
            continue
        cands.sort(key=lambda pp: bu_score(d, role, pp), reverse=True)
        pick = cands[0]
        schedule[d_iso][role] = pick
        total_hours[pick] += 8
    # --- end greedy fallback ---
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched,
    ...
    - Loads template_xlsx to preserve sizing, borders, merges, fonts.
    - Writes ONLY values + fills into day blocks (no border/width/height drawing).
    - Rebuilds Weekly Hours / Summary / Pay Period Hours / Violations sheets.
    """
    import openpyxl
    import calendar
    ...
    # Month title in your template is B1 (merged across)
    ws["B1"] = f"{calendar.month_name[month]} {year}"
    ...
    # Template is blank for fills; apply these
    ...
    for wk in range(grid_weeks):
        ...
        for dow in range(7):
            ...
            # Clear day block + apply fill (DO NOT TOUCH borders/sizing)
            ...
            # BU names only (no labels). Blank on Sundays.
            if not is_sunday(d):
    ...
    # Remove old report sheets if they exist (avoid duplicates)
    ...
    # Violations (uses your existing helper)
    push_days = push_days_for_month(year, month)
