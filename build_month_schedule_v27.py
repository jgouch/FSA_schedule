...
Logic Changes (v27):
- FIXED: Consolidate Violations loop (Hard Off, Avoid, B2B, Duplicates).
- FIXED: Pay Period Audit logic now matches Solver (Week-based scan).
- NEW: Audit for Duplicate Assignments on same day.
- CLEANUP: Removed unused MAX_WEEKS constant.
"""
...
def export_to_excel(schedule, year, month, out_path, roster, hols, prev_sched, payperiods, sales_order, cons):
    ...
    # 2. Hard Constraints & Duplicates Audit (Unified Loop)
    push_days = push_days_for_month(year, month)
    ...
    for d_iso, rmap in schedule.items():
        ...
        for r, p in rmap.items():
            ...
            if p in seen_today:
                ws_vio.append(["Duplicate Assignment", p, f"Assigned multiple roles on {d} (e.g. {r})"])
            seen_today.add(p)
            ...
            # Back-to-Back Check (Strict Role Matching)
    ...
    # 3. Hours Audit (Weekly + PP) using Parity Logic
    
    # Weekly check (Skip if Holiday or Push)
    for ws_date in week_starts:
    ...
    # PP Check (Skip if PP contains ANY Push Week)
    if payperiods:
        for s, e in payperiods:
            # Audit Logic: if PP intersects any Push Week, relax.
            # Scan weeks involved in this PP
            pp_is_relaxed = False
