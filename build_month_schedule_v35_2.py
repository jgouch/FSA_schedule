...
def build_schedule(config: BuildConfig,
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        sunday_pn_used = set()  # HARD: no one can be Sunday PN twice in the month
        ...
        for sd in sundays:
            ...
            for offset in range(len(config.roster)):
                ...
                if who in sunday_pn_used: continue  # HARD: prevent 2 Sundays for same person
    ...
    if not solve_bu(0):
        ...
        # --- end greedy fallback ---
    # print("Warning: Could not fill all desired BU slots.")
        # --- BU relabel pass (avoid same BU# back-to-back by swapping labels, without changing who works) ---
    def relabel_bu(schedule_dict):
        from itertools import permutations
        def bu_people(day_roles):
            ppl = []
            for k in ['BU1','BU2','BU3','BU4']:
                if k in day_roles:
                    ...
                    if v and str(v).strip():
                        ppl.append(v)
            return ppl
        day_isos = sorted(schedule_dict.keys())
        bu_sets = {d: bu_people(schedule_dict[d]) for d in day_isos}
        def clear_bu(d):
            for k in ['BU1','BU2','BU3','BU4']:
                schedule_dict[d].pop(k, None)
        def apply_perm(d, perm):
            clear_bu(d)
            for i, person in enumerate(perm, 1):
                schedule_dict[d][f'BU{i}'] = person
        def map_labels(d, perm):
            return {perm[i]: f'BU{i+1}' for i in range(len(perm))}
        def ok_adj(prev_map, cur_map):
            for person, lab in cur_map.items():
                if person in prev_map and prev_map[person] == lab:
                    return False
            return True
        # sliding 3-day window backtracking
        for i, d in enumerate(day_isos):
            if not bu_sets[d]:
                continue
            win = []
            for j in range(max(0, i-2), i+1):
                ...
                if bu_sets[dj]:
                    win.append(dj)
            # generate perms per day
            permlist = {dj: list(permutations(bu_sets[dj])) for dj in win}
            best = None
            def bt(idx, maps):
                nonlocal best
                if idx < 0:
                    ...
                    return True
                dj = win[idx]
                next_d = win[idx+1] if idx+1 < len(win) else None
                for perm in permlist[dj]:
                    cur_map = map_labels(dj, perm)
                    if next_d:
                        if not ok_adj(cur_map, maps[next_d]):
                            continue
                    maps[dj] = cur_map
                    if bt(idx-1, maps):
                        return True
                return False
            # seed maps with last day using each perm until success
            last = win[-1]
            for perm in permlist[last]:
                maps = {last: map_labels(last, perm)}
                if bt(len(win)-2, maps):
                    break
            if best:
                for dj in win:
                    # reconstruct perm in BU order
                    inv = sorted(best[dj].items(), key=lambda kv: int(kv[1].replace('BU',''))) 
                    perm = [person for person,_ in inv]
                    apply_perm(dj, perm)
...
def main():
    ...
    export_to_excel(
        sched, year, month, ox, roster, observed_holidays_for_year(year),
