...
Gridlines must be OFF. Borders ON.
"""
...
FONT_DAY_NUM = Font(name="Calibri", size=11, bold=True) # Bold day number
FONT_HEADER = Font(name="Calibri", size=24, bold=True) # Larger header
ALIGN_CENTER = Alignment(horizontal="center", vertical="center", wrap_text=True)

# Thin border for the grid
...
# CHANGED: Allow up to 6 weeks to support all calendar months
MAX_WEEKS = 6  
...
def build_schedule(config: BuildConfig,
    ...
    def would_exceed_week_days_cap(d: date, name: str) -> bool:
        ...
        if week_contains_push(ws):
            return False
        # Relax cap if it's a holiday week (user request)
    ...
    def would_exceed_payperiod_cap(d: date, role: str, name: str) -> bool:
        ...
        if not pp:
            return False
        # Relax pay-period cap during push-week weeks
    ...
    # Keep track of total hours for scoring
    total_hours_assigned: Dict[str, int] = {n: 0 for n in roster}

    def update_hours(name: str, d: date, role: str, add: bool):
        h = hours_for_role(d, role, observed_holidays)
        if add:
            total_hours_assigned[name] += h
        else:
            total_hours_assigned[name] -= h
    ...
    for sd, who0 in sunday_assignees.items():
        ...
        update_hours(chosen, sd, "PN", True)
        if need_sat_pair:
            schedule[sat.isoformat()]["AN"] = chosen
            update_hours(chosen, sat, "AN", True)
    ...
    def score_primary(d: date, role: str, name: str) -> float:
        ...
        # HEAVILY weight target adherence to fix distribution skew
        s += (t - c) * 20.0
        
        # Penalize total hours to balance Greg vs others
        s -= total_hours_assigned[name] * 0.5

        # Penalize current pay period hours to smooth paychecks
        pp = find_pay_period(d)
        if pp:
            s -= pay_period_hours(name, pp) * 1.0
    ...
    def candidates_for(slot: Tuple[date, str]) -> List[str]:
        ...
        random.shuffle(cands) # Randomize before sort to break ties
    ...
    def unassign_primary(d: date, role: str, name: str):
        ...
        role_counts[role][name] -= 1
        update_hours(name, d, role, False)
        if role == "PN":
            wd = weekday_sun0(d)
            pn_weekday_counts[name][wd] -= 1
            if wd == 1:
    ...
    if not ok:
        raise RuntimeError(f"❌ Failed to assign primary roles. Stuck at {last_fail_slot[0].isoformat()} role={last_fail_slot[1]} with 0 candidates.")

    # ---- BU backfill phase (Dynamic / Best Effort / Strict Sequencing) ----------------
    ...
    for d in days:
        ...
        # 1. Define the IDEAL BU roles for this specific day
        ideal_bu = []
        if d in push_days:
            if is_saturday(d):
            ...
            if weekday_sun0(d) == 1:  # Monday
                ideal_bu = ["BU1", "BU2", "BU3"]
            elif is_weekday(d): # Tue-Fri
                ideal_bu = ["BU1", "BU2"]
            else:
                # Sat/Sun non-push: NO BU roles
                ideal_bu = []

        if not ideal_bu:
            continue

        # 2. Strict Sequential Filling
        # We try to fill BU1. If successful, we proceed to BU2.
        # If we cannot fill BU1 (e.g. everyone available has "Avoid BU1"), we STOP.
        # We do NOT skip to BU2.
        
        assigned_today = set(schedule[d.isoformat()].values())
        hard_off_today = cons.hard_off.get(d, set())
        
        # Initial pool of available people (names only)
        available_pool = {p for p in roster if p not in assigned_today and p not in hard_off_today}
        
        # We simulate filling slots to see how many we can create. 
        # Note: The solver will do the actual assignment, but we must ensure a solution exists.
        # However, since the solver backtracks, simply knowing "someone" matches is enough to CREATE the slot.
        # But to enforce "No BU2 without BU1", we need to know if BU1 is *possible*.
        
        current_pool = available_pool.copy()
        
        for role_name in ideal_bu:
            # Check if there is at least one person in current_pool who does NOT avoid this role
            candidates = [p for p in current_pool if p not in cons.avoid_roles.get((d, role_name), set())]
            
            if not candidates:
                # Cannot fill this role with available people. STOP sequence.
                break
            
            # If we can fill it, we add the slot.
            # We don't remove a person yet because we don't know WHO the solver will pick.
            # But we know the capacity exists.
            # Wait - if we have 2 roles and 1 person, we can only fill 1.
            if len(current_pool) == 0:
                break

            if role_name not in schedule[d.isoformat()]:
                bu_slots.append((d, role_name))
                # We conceptually "use" one person capacity. 
                # Since we don't know who, we can't remove a specific person.
                # But we know we need 1 more body.
                # In the worst case (1 candidate), that candidate is used.
                # This heuristic is slightly loose but 'bu_slots' is just a target list.
                # The solver will fail if it can't fill it, but that's fine.
                # To be safe, we just decrement the *count* of available people.
                # But we must ensure specific constraints don't block. 
                # Since we checked `if candidates`, we know *someone* exists.
                # We'll just assume we consume one generic "capacity unit".
                # NOTE: This isn't perfect but strictly better than before.
                # To be perfect, we'd need max-flow, but sticking to greedy generation is okay here.
                pass 
            
            # Decrease theoretical capacity (count)
            # Actually, to prevent over-promising, we should remove the 'candidates' from consideration? 
            # No, that's too aggressive. Just ensure count > 0.
            # The simplified check:
            if len(candidates) > 0:
                 # Reduce pool size estimate
                 pass 
            # Actually, let's just rely on the solver. The critical fix is "Break if candidates is empty".
            # But we must also ensure we don't generate more slots than people.
            if len(bu_slots) >= len(available_pool) + len(assigned_today): # Total slots vs Total people
                 break
                 
            # Re-eval: The simplest strict logic:
            # We need to fill ideal_bu[i]. We have N people available.
            # If we generate slot, we need N >= 1.
            # Constraint check is key.
            
    # Refined logic for loop above to be safe and simple:
        # Re-calculate available count
        available_count = len(available_pool)
        
        # We will generate at most 'available_count' slots.
        # And we stop at the first role we can't find a candidate for.
        
        # This is complex because "Candidate for BU1" might be different from "Candidate for BU2".
        # We will just generate slots sequentially up to available_count, 
        # filtering by "Is there ANYONE who can do this role?"
        
        slots_to_create = []
        temp_pool = available_pool.copy()
        
        for role_name in ideal_bu:
            if not temp_pool:
                break
            
            # Find possible candidates for this specific role from the remaining pool
            valid_cands = [p for p in temp_pool if p not in cons.avoid_roles.get((d, role_name), set())]
            
            if not valid_cands:
                # Critical: If we can't fill BU1, we stop. We don't skip to BU2.
                break
            
            # We assume we can fill it. Add to list.
            slots_to_create.append(role_name)
            
            # "Reserve" one person (heuristically, just reduce count)
            # Since we don't know who the solver picks, we just pop one arbitrary valid candidate 
            # to ensure we don't double-count capacity.
            temp_pool.remove(valid_cands[0])
            
        for r in slots_to_create:
            bu_slots.append((d, r))
    ...
    def score_bu(d: date, role: str, name: str) -> float:
        s = rng.random()
        # Add balancing penalties to BU too
        s -= total_hours_assigned[name] * 0.5
        pp = find_pay_period(d)
        if pp:
            s -= pay_period_hours(name, pp) * 1.0
    ...
    def solve_bu(slots_left: List[Tuple[date, str]]) -> bool:
        ...
        for name in cands:
            nodes2 += 1
            schedule[d.isoformat()][role] = name
            update_hours(name, d, role, True) # Track hours
            nxt = slots_left[:idx] + slots_left[idx+1:]
            if solve_bu(nxt):
                return True
            del schedule[d.isoformat()][role]
            update_hours(name, d, role, False) # Revert hours
        return False

    ok2 = solve_bu(bu_slots)
    if not ok2:
        raise RuntimeError(f"❌ Failed to assign BU roles. Stuck at {last_fail_slot_bu[0].isoformat()} role={last_fail_slot_bu[1]} with 0 candidates.")
...
def export_schedule_to_excel(schedule: Dict[str, Dict[str, str]],
    ...
    # Optionally adjust row height if needed, or leave fixed 18
    ...
    for week_index in range(grid_weeks):
        ...
        for dow in range(7):
            ...
            # Apply fill AND border to all 24 cells of block
            for rr in range(R, R + WEEK_HEIGHT):
                for cc in range(col_start, col_start + DAY_WIDTH):
                    cell = ws.cell(rr, cc)
                    cell.font = FONT_MAIN
                    cell.alignment = ALIGN_CENTER
                    cell.border = THIN_BORDER # ADDED BORDER
                    if fill:
                        cell.fill = fill
            ...
            d = date(year, month, daynum)
            day_cell = ws.cell(R, col_end, daynum)
            day_cell.font = FONT_DAY_NUM # ADDED BOLD FONT
...
def main():
    ...
    # Logic: Generate periods based on the anchor, but do NOT clip the end date.
    if args.payperiods_json:
        ...
        # Filter: keep if it overlaps with the current month at all
        payperiods = []
