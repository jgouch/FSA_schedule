...
def build_schedule(config: BuildConfig,
    ...
    for d in days:
        ...
        # 2. Filter Ideal Roles based on WHO CAN ACTUALLY FILL THEM
        # This prevents generating a slot that exists on paper but is impossible 
        # because the only available person has an "avoid" rule for it.
        
        assigned_today = set(schedule[d.isoformat()].values())
        hard_off_today = cons.hard_off.get(d, set())
        
        # Identify pool of available people
        available_people = [p for p in roster if p not in assigned_today and p not in hard_off_today]
        
        # Select subset of roles that can be filled by at least one available person
        possible_roles = []
        for role_name in ideal_bu:
            # Is there ANY available person who does NOT avoid this role?
            can_fill = False
        ...
        # 3. Generate slots based on capacity
        # We can fill at most len(available_people) slots
        count_to_fill = min(len(possible_roles), len(available_people))
        
        # Take the top N possible roles
...
def export_schedule_to_excel(schedule: Dict[str, Dict[str, str]],
    ...
    row_height = 18 if grid_weeks == 5 else 15

    for col in range(2, 23):  # B..V
        ws.column_dimensions[get_column_letter(col)].width = 10

        # Grid height is dynamic based on grid_weeks.
...
def main():
    ...
    payperiods = load_payperiods_from_json(args.payperiods_json) if args.payperiods_json else (
        [(s, min(e, date(year, month, calendar.monthrange(year, month)[1]))) for (s, e) in generate_payperiods(
            datetime.strptime(args.payperiod_anchor_start, "%Y-%m-%d").date(),
            date(year, month, 1) - timedelta(days=28),
            date(year, month, calendar.monthrange(year, month)[1]) + timedelta(days=28)
        ) if (s <= date(year, month, calendar.monthrange(year, month)[1]) and e >= date(year, month, 1))]
    )
