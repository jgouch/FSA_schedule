...
def validate_staffing(
    ...
    min_weekday_staff_only_when_available: bool = True,
) -> Tuple[bool, str]:
    for d in month_days(year, month):
        if d in hols or not is_weekday(d):
            continue

        available = [p for p in roster if p not in cons.hard_off.get(d, set())]
        assigned_people = set(schedule.get(d.isoformat(), {}).values())

        if min_weekday_staff_only_when_available and len(available) < min_weekday_staff:
            continue

        if len(assigned_people) < min_weekday_staff:
            return (
                False,
                f"Understaffed {d}: assigned {len(assigned_people)} < {min_weekday_staff} with {len(available)} available",
            )
...
def main():
    ...
    ap.add_argument("--min-weekday-staff-only-when-available", action="store_true",
                    help="Only enforce min weekday staffing when at least that many counselors are available")
    ap.add_argument("--no-min-weekday-staff-only-when-available", action="store_false", dest="min_weekday_staff_only_when_available",
                    help="Enforce min weekday staffing on all non-holiday weekdays")
    ap.set_defaults(min_weekday_staff_only_when_available=True)
    args = ap.parse_args()
    ...
    if not args.auto_seed:
        ...
        for seed in range(args.seed_start, args.seed_end + 1):
            ...
            ok, reason = validate_staffing(
                ...
                min_weekday_staff_only_when_available=args.min_weekday_staff_only_when_available,
            )
            if ok:
                sched = candidate
                winning_seed = seed
