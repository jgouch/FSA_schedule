...
Logic Changes (v16):
- Split Relaxations: Holiday weeks relax day-caps; Push weeks relax PP-caps.
- Solver Engine: Restored MRV (Minimum Remaining Values) heuristic for stability.
- BU Compaction: Fixed gap logic to strictly remove empties.
"""
...
def compute_observed_holidays_us(year: int) -> Set[date]:
    ...
    out.add(d + timedelta(days=21)) # Thanksgiving
...
def build_schedule(config: BuildConfig,
    ...
    def can_assign_primary(d, role, name, strict_mode):
        ...
        # Rule: Weekly Caps apply unless it is a Holiday OR Push week
        if not (is_holiday_week or is_push_week):
            wh = week_hours(name, d)
            if wh + hours_for_role(d, role, hols) > 40: return False
        
        # Rule: 5-Day Cap applies unless it is a Holiday week
        if not is_holiday_week:
            ...
            # Current day counts as +1
            if days_worked >= 5: return False

        # Rule: Pay Period Cap applies unless it is a Push week
        if not is_push_week:
    ...
    def run_solver(strict_mode: bool) -> bool:
        ...
        for i, sd in enumerate(sundays):
            ...
            # Skip if hard off, find next available
            attempts = 0
            while who in cons.hard_off.get(sd, set()) and attempts < len(config.roster):
                start_idx += 1
                who = config.roster[(start_idx + i) % len(config.roster)]
                attempts += 1
            
            # Forcing Sunday PN even if caps hit (it's a hard rotation), 
            # unless strict mode says no quota. 
            # We'll allow it generally, but update counters.
            schedule[sd.isoformat()]["PN"] = who
            role_counts["PN"][who] += 1
            total_hours[who] += 5
            pn_weekday_counts[who][0] += 1
            
            # Try to pair with Saturday AN
        ...
        # 2. Assign remaining Primaries (MRV - Most Constrained Variable)
        primary_slots = []
        for d in days:
            req = roles_required_for_day(d, push_days, hols)
            for r in ["PN", "AN", "W"]:
        ...
        def backtrack(slots_left):
            ...
            # Heuristic: Pick slot with fewest valid candidates
            slots_left.sort(key=count_candidates)
            d, role = slots_left[0]
            ...
            for p in cands:
                ...
                if backtrack(slots_left[1:]): return True
    ...
    def can_bu(d, role, p):
        ...
        ws = week_start_sun(d)
        is_relaxed = week_contains_holiday(ws) or week_contains_push(ws)
        if not is_relaxed:
             wh = week_hours(p, d)
    ...
    def compact_bu_roles(sched):
        for d_iso, roles in sched.items():
            ...
            # Extract BU assignments
            bus = []
            for r in ["BU1", "BU2", "BU3", "BU4"]:
                if r in roles:
                    if roles[r]: # Ensure strictly not None/Empty
                        bus.append(roles[r])
                    del roles[r]
            
            # Re-insert sequentially
