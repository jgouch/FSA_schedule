...
- FIX (v56): v55 polish bundle — memoize week checks, remove signature drift, and enforce global BU1 precedence normalization.
- FIX (v57): Weekday target repair now fills multiple expected BU slots per eligible day (BU1 then BU2) to reach target staffing when feasible.
- FIX (v58): weekday_target_repair_pass counts attempts_for_day for direct BU attempts so per-day cap and verbose logs are accurate.
- FIX (v59): Add primary swap micro-pass (like-for-like PN/AN/W swaps across weekdays) to free weekly capacity and enable filling missing BU slots on target weekdays.
"""
