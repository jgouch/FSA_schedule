...
Logic Changes (v30):
- CLEANUP: Physically removed unused MAX_WEEKS constant.
- DOCS: Added policy clarification comments to Back-to-Back audit.
- STABILITY: Retained global 'calculate_daily_hours' source of truth.
"""
...
def write_violations_tab(wb, schedule, year, month, roster, targets, cons, prev_sched, payperiods, push_days, hols, week_starts):
    ...
    for d_iso in sorted_dates:
        ...
        for r, p in rmap.items():
            ...
            # NOTE: "Same Role" means STRICT role matching (e.g. PN->PN). 
            # It does NOT forbid PN->BU2. This is intentional to separate
            # 'exhaustion' from 'role variety' concerns.
