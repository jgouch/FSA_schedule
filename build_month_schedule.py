...
def spinner_line(phase: str, nodes: int, rate: float, extra: str = "") -> str:
    return f"| [{phase}] nodes={nodes:,} rate={rate:,.0f}/s {extra}".ljust(120)
...
def load_timeoff_from_xlsx(xlsx_path: str, sheet_name: str) -> List[TimeOffRule]:
    ...
    - Date: Excel date or YYYY-MM-DD
    - Hard: TRUE/FALSE or 1/0 or blank (blank => TRUE)
    - AvoidRoles: comma-separated like "PN,AN" (optional)
    """
    ...
    def col(name: str) -> int:
        k = name.lower()
        if k not in headers:
            raise ValueError(f"TimeOff sheet missing column '{name}' (headers row 1). Found: {list(headers.keys())}")
        return headers[k]

    c_date = col("date")
    c_name = col("name")
    c_hard = headers.get("hard", None)
    c_avoid = headers.get("avoidroles", None)
...
def build_schedule(config: BuildConfig,
                   ...
                   cons: Constraints) -> Dict[str, Dict[str, str]]:
    ...
    def can_assign_primary(d: date, role: str, name: str) -> bool:
        ...
        if name in schedule[d.isoformat()].values():
            return False
        # hard: no same specific role back-to-back day (across boundary)
        prev_role = last_role(name, d)
        if prev_role == role:
            return False
        # hard: cannot work > max_consecutive_days in a row (across boundary)
        if consecutive_streak(name, d) >= config.max_consecutive_days:
            return False
        # hard: Monday PN max 1 per person per month
    ...
    if not ok:
        raise RuntimeError("❌ Failed to assign primary roles under current constraints.")
    ...
    def can_assign_bu(d: date, role: str, name: str) -> bool:
        ...
        if name in schedule[d.isoformat()].values():
            return False
        # prevent same specific role back-to-back
        prev_role = last_role(name, d)
        if prev_role == role:
            return False
        # keep the same max consecutive days rule (still relevant operationally)
        if consecutive_streak(name, d) >= config.max_consecutive_days:
            return False
        return True
    ...
    if not ok2:
        raise RuntimeError("❌ Failed to assign BU roles under current constraints.")
...
def main():
    ...
    sched = build_schedule(cfg, prev, cons)
