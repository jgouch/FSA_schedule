...
Logic Changes (v28):
- REFACTOR: Extracted 'write_violations_tab' helper for clean, safe audit logic.
- FIXED: Duplicate assignment audit now reports specific conflicting roles.
- CLEANUP: Removed MAX_WEEKS constant.
- OPTIMIZED: Computed push_days once during export.
"""
...
def write_violations_tab(wb, schedule, year, month, roster, targets, cons, prev_sched, payperiods, push_days, hols, week_starts):
    ...
    def get_hours_on_day(dd, name):
        # Local helper re-using logic from main scope roughly
        # But we need access to 'hours_for_role' which is global
        if dd.year == year and dd.month == month:
            roles = schedule.get(dd.isoformat(), {})
            dh = 0
            for r, n in roles.items():
    ...
    for d_iso in sorted_dates:
        ...
        for r, p in rmap.items():
            ...
            if p in avoid_list:
                rtype = "Hard" if r in ROLE_PRIMARY else "Soft"
                ws_vio.append([f"Avoid Violation ({rtype})", p, f"Assigned {r} on {d}"])
                
            # D. Back-to-Back (Strict Role Key)
    ...
    for ws_date in week_starts:
        ...
        for p in roster:
            ...
            for i in range(7):
                ...
                tot += get_hours_on_day(dd, p)
            if tot > 40:
                ws_vio.append(["Weekly Cap Violation", p, f"{tot}h in week of {ws_date}"])
    ...
    if payperiods:
        for s, e in payperiods:
            ...
            for p in roster:
                ...
                cur = s
                while cur <= e:
                    h += get_hours_on_day(cur, p)
                    cur += timedelta(days=1)
                if h > 80:
                    ws_vio.append(["PayPeriod Cap Violation", p, f"{h}h in PP {s}:{e}"])
